...
//        	{
//        		parent.Public.tips({type:2, content : '共享使用者已經達到上限值：'+json.totalUserNum});
//        		return false;
//        	}else
//        	{
//        		window.location.href='../noteprinttemp/add';
  ...
  (function($){
    ...
    $('.grid-wrap').on('click', '.delete', function(e){
      ...
      $.ajax({
        ...
        success: function(data){
          if (data.status == 200) {
            parent.Public.tips({content: '取消使用者授權成功！'});
            usedTotal--;
            leftTotal++;
            showShareCount();
            if (rowData.isCom) {
            ...
            parent.Public.tips({type: 1, content: '取消使用者授權失敗！' + data.msg});
          }
        },
        error: function(){
           parent.Public.tips({content:'取消使用者授權失敗！請重試。', type: 1});
        }
      });
    });
    ...
    $('.grid-wrap').on('click', '.authorize', function(e){
      ...
      e.preventDefault();
       $.ajax({
        ...
        success: function(data){
          if (data.status == 200) {
            parent.Public.tips({content : '授權成功！'});
            rowData.share = true;
            $("#grid").jqGrid('setRowData', id, rowData);
            usedTotal++;
            leftTotal--;
        ...
        error: function(){
          parent.Public.tips({type:1, content : '使用者授權失敗！請重試。'});
        }
      });
    });
        ...
        if (val || row.admin) {
          if (row.admin) {
              return '管理員';
               ...
               return '<div class="operating" data-id="' + row.userId + '"><span class="delete ui-label ui-label-success">已啟用</span></div>';
          }
        } else {
    ...
    function settingFormatter(val, opt, row) {
		if (row.admin || row.share === false) {
			...
			return '<div class="operating" data-id="' + row.userId + '"><a class="ui-icon ui-icon-pencil" title="詳細設定授權資訊" href="../settings/authority_setting?userName=' + row.userName + '"></a></div>';
		}
    };
    function edit(val, opt, row) {
		if (row.admin || row.share === false) {
		    ...
		    var d = '<div class="operating" data-id="' + row.id + '"><a class="ui-icon ui-icon-pencil" href="../Noteprinttemp/edit?id=' + row.id + '" title="修改"></a><a class="ui-icon ui-icon-trash" title="刪除" href="../Noteprinttemp/del?id=' + row.id + '"></a></div>';
			return d
			//return '<a class="ui-icon ui-icon-pencil" title="詳細設定授權資訊" href="../settings/Noteprinttemp/edit?id=' + row.id + '"></a> <a class="ui-icon ui-icon-trash" title="刪除"></a>';
		}
    };
