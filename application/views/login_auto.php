        ...
        <div class="LoginBox">
            ...
            <div class="Loginc">
                <div class="loginInputc">
                    <input type="text" class="logininput" value="<?php echo get_cookie('username')?>" id="username" placeholder="輸入域賬號" />
                </div>
                <div class="loginInputc">
                    <input type="password" class="logininput" id="password" value="<?php echo get_cookie('userpwd')?>" placeholder="輸入域密碼" />
                </div>
            </div>
            <div style="color:red; font-size:20px; margin-top:12px; line-height:20px; display:none;" id="loginerror"></div>
            <div class="checkMMBox">
                <span class="checkbox<?php echo get_cookie('ispwd')==1?'true':'false'?>" id="Checked" onClick="ChechBoxAction(this)" val="<?php echo get_cookie('ispwd')?>">記住賬號</span>
            </div>
...
<!--需要loading 的頁面就在頁面最下方加-->
<div class="loading">
	<img src="<?php echo base_url()?>statics/login/<?php echo sys_skin()?>/Images/loading.gif" style="position:absolute;top:50%;left:50%;margin:-82px 0 0 -135px;" alt="請稍後...">
</div>
<script type="text/javascript">
    ...
    function Login() {
        ...
        if (!cookieEnabled) {
            alert("該瀏覽器Cookie設定不正確，無法正常登錄");
            return false;
        }
        var username = $.trim($("#username").val());
        var password = $.trim($("#password").val());
        var isRemmenbPassWord = $("#Checked").attr("val"); // 1為記住密碼  0 未記住密碼
        if (checkNullOrEmpty(username)) {
            $("#loginerror").text("請輸入賬號").show();
            $("#username").focus();
            return false;
        }
        if (checkNullOrEmpty(password)) {
            $("#loginerror").text("請輸入密碼").show();
            $("#password").focus();
            return false;
        }
        $('.loading').show();
...
//                }
            ...
            error: function (xhr, status) {
                if (status == "timeout") {
                    $("#loginerror").text("您的網路好像很糟糕，請重新整理頁面重試").show();
                    $('.loading').hide();
                    return false;
                }
                else {
                    $("#loginerror").text("伺服器內部錯誤，請重試").show();
                    $('.loading').hide();
                    return false;
                }
            }
