...
class InvPu extends CI_Controller {
	...
	public function exportInvPu(){
		...
		sys_csv($name);
		$this->common_model->logs('導出採購單據:'.$name);
		$sidx = str_enhtml($this->input->get_post('sidx',TRUE));
		$sord = str_enhtml($this->input->get_post('sord',TRUE));
		$transType = intval($this->input->get_post('transType',TRUE));
		$matchCon  = str_enhtml($this->input->get_post('matchCon',TRUE));
	...
	public function findUnhxList(){
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['transType']    = $row['transType']==150501 ? '購貨' : '退貨';
			$v[$arr]['billDate']     = $row['billDate'];
	...
	public function add(){
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->common_model->logs('新增購貨 單據編號：'.$info['billNo']);//.'，報文：'.$_POST['postData']//stripslashes(json_encode($info))
	...
	public function updateInvPu(){
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('修改購貨單 單據編號：'.$data['billNo']);
    ...
    public function delete() {
		...
		if (count($data)>0) {
		    foreach($data as $arr=>$row) {
			    $row['checked'] >0 && str_alert(-1,'其中已有審覈的不可刪除'); 
				$ids[]           = $row['id'];
				$billNo[]        = $row['billNo'];
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('刪除購貨訂單 單據編號：'.$billNo);
				str_alert(200,$msg); 	 
			}
		}
		str_alert(-1,'單據不存在');  
	...
	public function checkInvPu() {
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('採購單據編號：'.$data['billNo'].'的單據已被審覈！');
				str_alert(200,'success',array('id'=>$data['id'])); 
			}
		}
		str_alert(-1,'提交的數據不能為空'); 
    ...
    public function batchCheckInvPu() {
		...
		if (count($data)>0) {
		    foreach($data as $arr=>$row) {
			    $row['checked'] > 0 && str_alert(-1,'勾選當中已有審覈，不可重複審覈'); 
			    $ids[]        = $row['id'];
				$billNo[]     = $row['billNo'];
			...
			if ($sql) {
				...
				$this->common_model->logs('購貨單編號：'.$billNo.'的單據已被審覈！');
				str_alert(200,'單據編號：'.$billNo.'的單據已被審覈！');
			} 
			str_alert(-1,'審覈失敗');  
		}
		str_alert(-1,'單據不存在！'); 
    ...
    public function rsBatchCheckInvPu() {
	    ...
	    $id   = str_enhtml($this->input->post('id',TRUE));
	    $this->mysql_model->get_count('verifica_info','(billId='.$id.')')>0 && str_alert(-1,'存在關聯的「付款單據」，無法刪除！請先在「付款單」中刪除該銷貨單！');//add
		$data = $this->mysql_model->get_results('invoice','(id in('.$id.')) and billType="PUR" and isDelete=0');  
		if (count($data)>0) {
		    foreach($data as $arr=>$row) {
			    $row['checked'] < 1 && str_alert(-1,'勾選當中已有未審覈，不可重複反審覈'); 
				$ids[]        = $row['id'];
				$billNo[]     = $row['billNo'];
				$srcOrderId[] = $row['srcOrderId'];
			}
			...
			if ($sql) {
				...
				$this->common_model->logs('購貨單單號：'.$billNo.'的單據已被反審覈！');
				str_alert(200,'購貨單編號：'.$billNo.'的單據已被反審覈！'); 
			} 
			str_alert(-1,'反審覈失敗');  
		}
		str_alert(-1,'單據不存在！'); 
	...
	private function validform($data) {
		...
		$data['billType']        = 'PUR';
		$data['transTypeName']   = $data['transType']==150501 ? '購貨' : '退貨';
		...
		strlen($data['billNo']) < 1 && str_alert(-1,'單據編號不為空');  
		...
		$this->mysql_model->get_count('contact',array('id'=>$data['buId'])) < 1 && str_alert(-1,'購貨單位不存在');   
	...
	private function account_info($iid,$data) {
		foreach ($data['accounts'] as $arr=>$row) {
			...
			$v[$arr]['billType']          = $data['billType'];
			$v[$arr]['transType']         = $data['transType'];
			$v[$arr]['transTypeName']     = $data['transType']==150501 ? '普通採購' : '採購退回'; 
	...
	public function addImagesToInv() {
		...
		if (strlen($data)>0) {
		    ...
		    !isset($data['files']) || count($data['files']) < 1 && str_alert(-1,'請先新增圖片！'); 
