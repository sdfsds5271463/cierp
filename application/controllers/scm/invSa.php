...
class InvSa extends CI_Controller {
	...
	public function findUnhxList(){
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['transType']    = $row['transType']==150601 ? '銷貨' : '退貨';
			$v[$arr]['billDate']     = $row['billDate'];
	...
	public function checkInvSa() {
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('銷貨單據編號：'.$data['billNo'].'的單據已被審覈！');
				str_alert(200,'success',array('id'=>$data['id'])); 
			}
		}
		str_alert(-1,'提交的數據不能為空'); 
    }
	...
	public function revsCheckInvSa() {
		...
		if (strlen($data)>0) {
		    $data = $this->validform((array)json_decode($data, true)); 
		    //$this->mysql_model->get_count('verifica_info','(billId='.$data['id'].')')>0 && str_alert(-1,'存在關聯收款單據，無法刪除！請先在收款單中刪除該銷貨單！');
			...
			if ($sql) {
				...
				$this->common_model->logs('購貨單據編號：'.$data['billNo'].'的單據已被反審覈！');
				str_alert(200,'success',array('id'=>$data['id'])); 
			}
			str_alert(-1,'SQL錯誤'); 
		}
    ...
    public function batchCheckInvSa() {
		...
		if (count($data)>0) {
			...
			if ($sql) {
				...
				$this->common_model->logs('銷貨單編號：'.$billno.'的單據已被審覈！');
				str_alert(200,'銷貨單編號：'.$billno.'的單據已被審覈！');
			} 
			str_alert(-1,'審覈失敗');  
		}
		str_alert(-1,'所選的單據都已被審覈，請選擇未審覈的銷貨單進行審覈！'); 
	}
    ...
    public function rsBatchCheckInvSa() {
	    $this->common_model->checkpurview(90);
	    $id   = str_enhtml($this->input->post('id',TRUE));
	    $this->mysql_model->get_count('verifica_info','(billId='.$id.')')>0 && str_alert(-1,'存在關聯「收款單據」，無法刪除！請先在「收款單」中刪除該銷貨單！');//add
		...
		if (count($data)>0) {
			...
			if ($sql) {
				...
				$this->common_model->logs('銷貨單：'.$billno.'的單據已被反審覈！');
				str_alert(200,'銷貨單編號：'.$billno.'的單據已被反審覈！'); 
			} 
			str_alert(-1,'反審覈失敗');  
		}
		str_alert(-1,'所選的銷貨單都是未審覈，請選擇已審覈的銷貨單進行反審覈！'); 
	}
    ...
    public function delete() {
		...
		if (count($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('刪除購貨訂單 單據編號：'.$data['billNo']);
	...
	private function validform($data) {
		...
		$data['transTypeName']   = $data['transType']==150601 ? '銷貨' : '銷退';
		...
		$data['arrears'] < 0 && str_alert(-1,'本次欠款要為數字，請輸入有效數字！'); 
		$data['disRate'] < 0 && str_alert(-1,'折扣率要為數字，請輸入有效數字！'); 
		$data['rpAmount'] < 0  && str_alert(-1,'本次收款要為數字，請輸入有效數字！'); 
		$data['customerFree'] < 0 && str_alert(-1,'客戶承擔費用要為數字，請輸入有效數字！'); 
		$data['amount'] < $data['rpAmount']  && str_alert(-1,'本次收款不能大於折後金額！'); 
		$data['amount'] < $data['disAmount'] && str_alert(-1,'折扣額不能大於合計金額！'); 
		...
		foreach ($data['accounts'] as $arr=>$row) {
			(float)$row['payment'] < 0 && str_alert(-1,'結算金額要為數字，請輸入有效數字！');
		}  
		...
		//供應商驗證
		$this->mysql_model->get_count('contact',array('id'=>$data['buId']))<1 && str_alert(-1,'客戶不存在'); 
		...
		foreach ($data['entries'] as $arr=>$row) {
			...
			if ($system['requiredCheckStore']==1 && $data['id']<1) {  
				 if (intval($data['transType'])==150601) {                        //銷售才驗證 
					if (isset($inventory[$row['invId']][$row['locationId']])) {
					    
						$inventory[$row['invId']][$row['locationId']] < $row['qty'] && str_alert(-1,$row['locationName'].$row['invName'].'商品庫存不足！'); 
					} else {
					    //str_alert(-1,$row['invName'].'庫存不足！');
					    //add by michen 20170719 for 組合品庫存檢查
					    $gooddata = $this->mysql_model->get_results('goods','(id ='.$row['invId'].') and (isDelete=0)');
					    if(count($gooddata)>0){
					        $dopey = reset($gooddata);
					        if($dopey['dopey'] != 1){
					            str_alert(-1,'商品'.$row['invName'].'庫存不足！');
					        }else{
					            ...
					            if(count($sonlist)>0){
    					            foreach ($sonlist as $sonkey=> $sonrow){
    					                if($inventory[$sonrow['gid']][$row['locationId']] < $row['qty']*$sonrow['qty'])
    					                   str_alert(-1,'商品「'.$row['invName'].'」的子商品「'.$sonrow['name'].'」庫存不足！');
    					            }
					            }else{
					                str_alert(-1,'商品「'.$row['invName'].'」的子商品「'.$sonrow['name'].'」丟失，請檢查！');
					            }
					        }
					    }else{
	...
	private function account_info($iid,$data) {
		foreach ($data['accounts'] as $arr=>$row) {
			...
			$v[$arr]['billType']      = $data['billType'];
			$v[$arr]['transType']     = $data['transType'];
			$v[$arr]['transTypeName'] = $data['transType']==150601 ? '普通銷售' : '銷售退回';
			$v[$arr]['billDate']      = $data['billDate']; 
