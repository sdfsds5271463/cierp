...
class InvTf extends CI_Controller {
	...
	public function exportInvTf(){
		...
		sys_csv($name);
		$this->common_model->logs('導出調撥單據:'.$name);
		$matchCon  = str_enhtml($this->input->get_post('matchCon',TRUE));
		$beginDate = str_enhtml($this->input->get_post('beginDate',TRUE));
		$endDate   = str_enhtml($this->input->get_post('endDate',TRUE));
		$inLocationId    = intval($this->input->get_post('inLocationId',TRUE));
	...
	public function add(){
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->common_model->logs('新增調撥單編號：'.$info['billNo']);
	...
	public function updateInvTf(){
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('修改調撥單編號：'.$data['billNo']);
    ...
    public function delete() {
		...
		if (count($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('刪除調撥單 單據編號：'.$data['billNo']);
	...
	private function validform($data) {
		...
		foreach ($data['entries'] as $arr=>$row) {
			...
			intval($row['outLocationId']) < 1 && str_alert(-1,'請選擇調出倉庫倉庫！'); 
			intval($row['inLocationId']) < 1  && str_alert(-1,'請選擇調入倉庫倉庫！'); 
			intval($row['outLocationId']) == intval($row['inLocationId']) && str_alert(-1,'調出倉庫不能與調入倉庫相同！'); 
			...
			if ($system['requiredCheckStore']==1 && $data['id']<1) {  
				if (isset($inventory[$row['invId']][$row['outLocationId']])) {
					$inventory[$row['invId']][$row['outLocationId']] < (float)$row['qty'] && str_alert(-1,$row['outLocationName'].$row['invName'].'商品庫存不足！'); 
				} else {
