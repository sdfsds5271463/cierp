...
class Payment extends CI_Controller {
	...
	public function payList(){ 
		...
		foreach ($list as $arr=>$row) {
		    $v[$arr]['amount']       = (float)$row['rpAmount'];     //收款金額
			$v[$arr]['adjustRate']   = (float)$row['discount'];     //整單折扣
			$v[$arr]['deAmount']     = (float)$row['payment'];      //本次預收款
			$v[$arr]['billDate']     = $row['billDate'];
			$v[$arr]['bDeAmount']    = (float)$row['hxAmount'];     //本次覈銷
			$v[$arr]['id']           = intval($row['id']);
			$v[$arr]['hxAmount']     = (float)$row['hxAmount'];     //本次覈銷
	...
	public function export(){
		...
		sys_csv($name);
		$this->common_model->logs('導出付款單:'.$name);
		$matchCon  = str_enhtml($this->input->get_post('matchCon',TRUE));
		$beginDate = str_enhtml($this->input->get_post('beginDate',TRUE));
		$endDate   = str_enhtml($this->input->get_post('endDate',TRUE));
		...
		if (strlen($data)>0) {
			...
			if (is_array($data['entries']) && count($data['entries'])>0) {
			    ...
			    if(count($duplicate)!= count(array_unique($duplicate))){
			        ...
			        str_alert(-1,'存在重複的源單編號，請覈實！');
			    }
			    if($rpAmount != $amount || $data['payment']!=0){
			        $this->db->trans_rollback();
			        str_alert(-1,'付款金額總和與覈銷金額總和不相等，請覈實！');
			    }
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->common_model->logs('新增付款單 單據編號：'.$data['billNo']);
	...
	public function updatePayment(){
		...
		if (strlen($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('修改收款單 單據編號：'.$data['billNo']);
    ...
    public function update() {
		...
		if (count($data)>0) {
			...
			foreach ($list as $arr=>$row) {
				...
				$v[$arr]['payment']       = (float)$row['payment']>0 ? -abs($row['payment']) : abs($row['payment']); //特殊情況
    ...
    public function delete() {
		...
		if (count($data)>0) {
			...
			if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('刪除收款單 單據編號：'.$data['billNo']);
				str_alert(200,'success'); 	 
			}
		}
		str_alert(-1,'單據不存在,或已被刪除'); 
	}
	...
	private function validform($data) {
		...
		count($data['accounts']) < 1 && str_alert(-1,'提交的是空數據'); 
		...
		$this->mysql_model->get_count('contact',array('id'=>$data['buId']))<1 && str_alert(-1,'請選擇供應商，供應商不能為空！'); 

		//數據驗證
		foreach ($data['accounts'] as $arr=>$row) {
		    (float)$row['payment'] < 0 && str_alert(-1,'付款金額不能為負數！'); 
			$data['rpAmount'] += abs($row['payment']);
		} 
        /*foreach ($data['entries'] as $arr=>$row) {
		    (float)$row['nowCheck'] < 0 && str_alert(-1,'覈銷金額不能為負數！'); 
