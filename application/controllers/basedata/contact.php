...
class Contact extends CI_Controller {
	...
	public function getRecentlyContact(){
		...
		$where = '(isDelete=0)'; 
		$where .= ($transType==150501||$transType==150502) ? ' and type=10' :' and type=-10';//mody by michen 20170820 修正購貨退貨單預設供應商不正確
		$where .= $this->common_model->get_contact_purview();
	...
	public function add(){
		...
		$this->mysql_model->get_count('contact',array('isDelete'=>0,'type'=>$data['type'],'number'=>$data['number'])) > 0 && str_alert(-1,'編號重複');
		$data = elements(array(
	...
	public function update(){
		...
		$this->mysql_model->get_count('contact',array('id !='=>$data['id'],'isDelete'=>0,'type'=>$data['type'],'number'=>$data['number'])) > 0 && str_alert(-1,'編號重複');
		$info = elements(array(
					'name','number','amount','beginDate','cCategory',
					'cCategoryName','cLevel','cLevelName','linkMans'
					,'periodMoney','remark','type','difMoney'),$data,NULL);
	...
	public function delete(){
		...
		if (count($data) > 0) {
		    $info['isDelete'] = 1;
		    $this->mysql_model->get_count('invoice','(isDelete=0) and (buId in('.$id.'))')>0 && str_alert(-1,'不能刪除有業務往來的客戶或供應商！');
	...
	public function disable(){
		...
		if (strlen($id) > 0) { 
		    ...
		    if ($sql) {
				$this->common_model->logs('客戶'.$disable==1?'禁用':'啟用'.':ID:'.$id.'');
				str_alert(200,'success');
			}
		}
		str_alert(-1,'操作失敗');
	...
	private function validform($data) {
		...
		$data['cCategory'] < 1 && str_alert(-1,'類別名稱不能為空');
		return $data;
	}  
