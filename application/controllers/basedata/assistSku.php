...
class Assistsku extends CI_Controller {
	...
	public function add(){
		...
		if (count($data)>0) {
			$this->mysql_model->get_count('assistsku',array('skuAssistId'=>$data['skuAssistId'])) > 0 && str_alert(-1,'輔助屬性組已存在！');
			$sql = $this->mysql_model->insert('assistsku',$data);
			if ($sql) {
				$data['skuId'] = $sql;
				$this->common_model->logs('新增規格:'.$data['skuName']);
	...
	public function delete(){
		...
		if (count($data)>0) {
		    ...
		    if ($sql) {
				$this->common_model->logs('刪除規格:ID='.$id.' 名稱:'.$data['skuName']);
