...
class Deliveryaddr extends CI_Controller {
	...
	public function update(){
		...
		if (count($data)>0) {
			...
			if ($sql) {
				$data['id'] = $id;
				$this->common_model->logs('更新地址:ID='.$id.',名稱:'.$data['shortName']);
				str_alert(200,'success',$data);
			}
		}
		str_alert(-1,'更新失敗');
	...
	public function delete(){
		...
		if (count($data)>0) {
		    //$this->mysql_model->get_count(INVSA,'(contactid in('.$id.'))')>0 && str_alert(-1,'其中有客戶發生業務不可刪除');
			$sql = $this->mysql_model->update('address',array('isDelete'=>1),array('id'=>$id));    
		    if ($sql) {
				$this->common_model->logs('刪除地址:ID='.$id.' 名稱:'.$data['shortname']);
