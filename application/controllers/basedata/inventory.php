...
class Inventory extends CI_Controller {
	...
	public function index() {
		...
		foreach ($list as $arr=>$row) {
			...
			$v[$arr]['currentQty']    = $row['totalqty'];                            //目前庫存
	...
	public function getNextNo() {
		...
		$this->mysql_model->get_count('goods',array('isDelete'=>0,'number'=>$skey)) > 0 && str_alert(-1,'商品編號已經存在');
	...
	public function checkBarCode() {
		 ...
		 $this->mysql_model->get_count('goods',array('isDelete'=>0,'barCode'=>$barCode)) > 0 && str_alert(-1,'商品條碼已經存在');
		 str_alert(200,'success');
	}
	...
	public function checkSpec() {
		 ...
		 $this->mysql_model->get_count('assistsku',array('isDelete'=>0,'skuName'=>$spec)) > 0 && str_alert(-1,'商品規格已經存在');
		 str_alert(200,'success');
	}
	...
	public function add(){
		...
		if ($data) {
			...
			$this->mysql_model->get_count('goods',array('isDelete'=>0,'number'=>$data['number'])) > 0 && str_alert(-1,'商品編號重複');
			$this->db->trans_begin();
			$info = array(
	...
	public function update(){
		...
		if ($data) {
			$data = $this->validform($data);
			$this->mysql_model->get_count('goods',array('id !='=>$data['id'],'isDelete'=>0,'number'=>$data['number'])) > 0 && str_alert(-1,'商品編號重複');
			$this->db->trans_begin();
			$info = array(
			    'barCode','baseUnitId','unitName','categoryId','categoryName','propertys',
				'discountRate1','discountRate2','highQty','locationId','pinYin',
            ...
            if ($this->db->trans_status() === FALSE) {
			    ...
			    $this->db->trans_commit();
				$this->common_model->logs('修改商品:ID='.$data['id'].'名稱:'.$data['name']);
	...
	public function delete(){
		...
		if (count($data) > 0) {
		    $this->mysql_model->get_count('invoice_info','(invId in('.$id.')) and (isDelete=0)')>0 && str_alert(-1,'其中有商品發生業務不可刪除');
		    $sql  = $this->mysql_model->update('goods',array('isDelete'=>1),'(id in('.$id.'))');   
		    if ($sql) {
			    $name = array_column($data,'name'); 
				$this->common_model->logs('刪除商品:ID='.$id.' 名稱:'.join(',',$name));
	...
	public function exporter() {
		...
		sys_csv($name);
		$this->common_model->logs('導出商品:'.$name);
		$skey = str_enhtml($this->input->get_post('skey',TRUE));
		$categoryid   = intval($this->input->get_post('assistId',TRUE));
		$barCode      = intval($this->input->get_post('barCode',TRUE));
		$where = '(a.isDelete=0)';
	...
	public function disable(){
		...
		if (strlen($id) > 0) { 
		    ...
		    if ($sql) {
				$this->common_model->logs('商品'.$disable==1?'禁用':'啟用'.':ID:'.$id.'');
				str_alert(200,'success');
			}
		}
		str_alert(-1,'操作失敗');
	...
	public function warningExporter() {
		...
		sys_csv($name);
		$this->common_model->logs('導出庫存預警商品:'.$name);
		$locationId  = intval($this->input->get_post('locationId',TRUE));
		$warnType    = intval($this->input->get_post('warnType',TRUE));
		$assistId    = intval($this->input->get_post('assistId',TRUE));
		$skey        = str_enhtml($this->input->get_post('skey',TRUE));
	...
	private function validform($data) {
	    ...
	    strlen($data['name']) < 1 && str_alert(-1,'商品名稱不能為空');
		strlen($data['number']) < 1 && str_alert(-1,'商品編號不能為空');
		$data['categoryId'] = intval($data['categoryId']);
		$data['baseUnitId'] = intval($data['baseUnitId']);
		$data['categoryId'] < 1 && str_alert(-1,'商品類別不能為空');
		$data['baseUnitId'] < 1 && str_alert(-1,'計量單位不能為空');
		...
		!$data['categoryName'] && str_alert(-1,'商品類別不存在');
	    if (strlen($data['propertys'])>0) {                            
			$list         = (array)json_decode($data['propertys'],true);
			...
			foreach ($list as $arr=>$row) {
				!in_array($row['locationId'],$locationId) && str_alert(-1,$locationName[$row['locationId']].'倉庫不存在或不可用！'); 
			} 
		}
