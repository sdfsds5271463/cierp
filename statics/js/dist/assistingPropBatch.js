	...
	THISPAGE = {
			...
			$("#grid").jqGrid({
				...
				colModel: [{
					...
					formatter: function(a, b, c) {
						var d = '<div class="operating" data-id="' + c.id + '"><span class="ui-icon ui-icon-trash" title="刪除"></span></div>';
						return d
					}
				}, {
					name: "skuName",
		...
		addEvent: function() {
			...
			$("#search").click(function() {
				...
				queryConditions.skuName = "輸入規格、屬性名稱查詢" === b ? "" : b, THISPAGE.reloadData(queryConditions)
				...
				if (a.preventDefault(), Business.verifyRight("FZSX_DELETE")) {
					...
					$.dialog.confirm("您確定要刪除該規格嗎？", function() {
