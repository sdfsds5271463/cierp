function initDom() {
	function a(a, b, c) {
		conditions.typeNumber = a, conditions.name = b, c || $("#grid").setGridParam({
			postData: conditions
		}).trigger("reloadGrid"), parent.$("li.l-selected a").eq(0).text(conditions.name + "分類")
	}
...
function initEvent() {
	$("#btn-add").click(function(a) {
		...
		conditions.skey = "輸入名稱查詢" == b ? "" : b, $("#grid").setGridParam({
			postData: conditions
		}).trigger("reloadGrid")
	...
	$("#grid").jqGrid({
		...
		loadComplete: function(a) {
			if (a && 200 == a.status) {
				...
				var f = 250 == a.status ? "沒有" + conditions.name + "屬性數據！" : "獲取" + conditions.name + "屬性數據失敗！" + a.msg;
				parent.Public.tips({
					type: 2,
					content: f
				})
...
var conditions = {
	...
	handle = {
		operate: function(a, b) {
			var c, d;
			"add" == a ? (c = "新增" + conditions.name + "屬性", d = {
				oper: a,
				typeNumber: conditions.typeNumber,
				callback: this.callback
			}) : (c = "修改" + conditions.name + "屬性", d = {
				oper: a,
				typeNumber: conditions.typeNumber,
		...
		del: function(a) {
			$.dialog.confirm("刪除的" + conditions.name + "屬性將不能恢復，請確認是否刪除？", function() {
				Public.ajaxPost("../basedata/assist/delete?action=delete", {
					...
					if (b && 200 == b.status) {
						parent.Public.tips({
							content: "刪除" + conditions.name + "屬性成功！"
						...
						type: 1,
						content: "刪除" + conditions.name + "屬性失敗！" + b.msg
					})
				})
			})
		},
