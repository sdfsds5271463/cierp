	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			if (this.$_customer = $("#customer"), 
				...
				parent.$("#page-tab").find("li.l-selected").children("a").html("銷貨退貨單");
				$("#paymentTxt").html("本次退款")
			} else {
				parent.$("#page-tab").find("li.l-selected").children("a").html("銷貨單");
			...
			if (this.salesCombo = Business.billSalesCombo($("#sales"), {
				...
				callback: {
					onChange: function(a) {
						//add by michen 20170724 begin
						if(a){
						var linkMen=$.parseJSON(a.linkMen);//json字串直接轉對像
						SYSTEM.linkinfo = linkMen;
						THISPAGE.linkCombo.loadData(linkMen);
				...
				for (var e = 0; e < SYSTEM.salesInfo.length; e++) if (SYSTEM.salesInfo[e].id === a.salesId) {
					...
					if ("" == b.customerCombo.input.val()) {
						...
						//add by michen 20170724 begin
						var linkMen=a.linkMen;//json字串直接轉對像
						SYSTEM.linkinfo = linkMen;
						linkMen=$.grep(linkMen,function(n,i){
							return n.linkFirst==1;
						});
						...
						THISPAGE.linkCombo = Business.linkCombo(THISPAGE);
						setTimeout(function(){THISPAGE.linkCombo.selectByValue(linkMan.linkName)},10);
						//add by michen 20170724 end
					}
			});
			...
			setTimeout(function(){THISPAGE.linkCombo.selectByValue(a.udf01)},10),*/
			//setTimeout(function(){THISPAGE.linkCombo.selectByValue(a.udf01)},10);
			//setTimeout(function(){THISPAGE.linkCombo.input.val(a.udf01)},1000);
			//a.udf01 && $("#linkMan").find("input").val(a.udf01),
			...
			var f = '<!--<a id="original" class="ui-btn ui-btn-sc">選擇源單</a>--><a id="savaAndAdd" class="ui-btn ui-btn-sp">儲存並新增</a><a id="save" class="ui-btn">儲存</a>',
				g = '<!--<a id="original" class="ui-btn ui-btn-sc">選擇源單</a>--><a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSa/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">列印</a><a id="edit" class="ui-btn">儲存</a>',
				h = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invSa/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">列印</a>',
				i = "",
				j = "",
				k = "";
			billRequiredCheck ? (i = '<a class="ui-btn" id="audit">審覈</a>', j = '<a class="ui-btn" id="reAudit">反審覈</a>') : this.$_checkName.parent().hide();
			var l = '<a class="ui-btn-prev" id="prev" title="上一張"><b></b></a><a class="ui-btn-next" id="next" title="下一張"><b></b></a>';
		...
		loadGrid: function(a) {
			...
			var B = [{
				...
				name: "goods",
				label: "商品",
				nameExt: '<span id="barCodeInsert">掃瞄槍錄入</span>',
				width: 300,
				classes: "goods",
				formatter: c,
				editable: !0,
			...
			w.mod_PageConfig.gridReg(C, B), B = w.mod_PageConfig.conf.grids[C].colModel, $("#grid").jqGrid({
				...
				afterEditCell: function(a, b, c, d, e) {
					...
					if (THISPAGE.curID = a, "goods" === b && (f(), $("#" + d + "_goods", "#grid").val(c), THISPAGE.goodsCombo.selectByText(c)), "skuName" === b) {
						...
						if ("string" == typeof g.invSkus && (g.invSkus = $.parseJSON(g.invSkus)), $("#" + d + "_skuName", "#grid").val(c), THISPAGE.skuCombo.loadData(g.invSkus || [], 1, !1), THISPAGE.skuCombo.selectByText(c), !g || "150602" == originalData.transType) return;
					...
					if ("price" === b && $("#" + d + "_price", "#grid").val(c), "locationName" === b) {
						...
						var g = $("#" + a).data("goodsInfo");
						if (!g || "150602" == originalData.transType) return;
						SYSTEM.ISSERNUM && 1 == g.isSerNum && Business.serNumManage({
							row: $("#" + a),
							enableStorage: !0,
							enableSku: !0,
			...
			function b() {
				c.$_customer.data("contactInfo", {
					...
					name: a.contactName
						...
						//add by michen 20170724 begin
						c.$linkMan.find("input").val(a.udf01),
						c.$linkPhone.find("input").val(a.udf02),
						c.$linkAddress.find("input").val(a.udf03)
						//add by michen 20170724 end
			}
			$("#grid").clearGridData();
			var c = this;
			...
			this.goodsCombo = Business.billGoodsCombo($(".goodsAuto"), {
				...
				data: function() {
					...
					if (d.prices.levelPrice) {
						var f = "";
						a.cLevel < 3 ? f = ["零售", "批發", "VIP"][a.cLevel] + "價：" + d.prices.levelPrice : d.prices.discountRate && (f = ["折扣一", "折扣二"][a.cLevel - 3] + "價：" + d.prices.levelPrice * d.prices.discountRate / 100), f && e.push({
							name: f,
							id: 1
						})
					}
					return d.prices.nearPrice && e.push({
						name: "最近售價：" + d.prices.nearPrice,
						id: 2
					}), e
				},
				text: "name",
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				if (c) {
					...
					THISPAGE.$_discountRate.val(e.toFixed(amountPlaces)), THISPAGE.$_discount.val(d), THISPAGE.$_arrears.val("-0.00" != f.toFixed(amountPlaces) ? f.toFixed(amountPlaces) : "0.00")
				}
				...
				THISPAGE.$_arrears.val("-0.00" != d.toFixed(amountPlaces) ? d.toFixed(amountPlaces) : "0.00"), THISPAGE.$_totalArrears.val(e.toFixed(amountPlaces));
				...
				if (a.preventDefault(), Business.verifyRight("SA_ADD")) {
					var b = "銷貨單",
						c = "sales-sales";
					if ("150602" == originalData.transType) var b = "銷貨退貨單",
						c = "sales-salesBack";
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已經沒有上一張了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("數據載入中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSa/update?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已經沒有下一張了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("數據載入中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invSa/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(a) {
		...
		getPostData: function(a) {
			...
			if (f.length > 0) {
				...
				if (taxRequiredCheck && (h.totalTax = $("#grid").jqGrid("footerData", "get").tax.replace(/,/g, ""), h.totalTaxAmount = $("#grid").jqGrid("footerData", "get").taxAmount.replace(/,/g, "")), requiredMoney) {
					...
					var i = "150601" == h.transType ? "收款額" : "退款額";
					if (0 !== Number(h.rpAmount) && 0 === h.accId) return parent.Public.tips({
						type: 1,
						content: i + "不為空時，請選擇結算賬戶！"
					}), !1;
					if (0 === Number(h.rpAmount) && 0 !== h.accId) return parent.Public.tips({
						type: 1,
						content: "結算賬戶不為空時，需要輸入" + i + "！"
					}), !1;
