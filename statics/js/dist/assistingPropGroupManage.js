				...
				for (var b = 0, f = a.length; f > b; b++) {
					...
					this.$comboField.append(g.join("")), function(c) {
						...
						e.combolist.push($("#" + c).combo({
							...
							defaultSelected: 0,
							editable: !0,
							extraListHtml: '<a href="javascript:void(0);" id="quickAddSku" class="quick-add-link"><i class="ui-icon-add"></i>新增屬性</a>',
							maxListWidth: 500,
							cache: !1,
							forceSelection: !0,
							listHeight: 100,
							listWrapCls: "ui-droplist-wrap",
							noDataText: "請先新增" + f + "屬性",
							callback: {
								onChange: function() {}
							}
						}).getCombo(a[b]))
			...
			initEvent: function() {
				$(".quick-add-link").on("click", function() {
					for (var a = 0, b = e.combolist.length; b > a; a++) if (e.combolist[a].active) var c = e.combolist[a],
						d = c.obj[0].id,
						f = "新增" + c.opts.noDataText.replace("請先新增", ""),
						g = parent.$.dialog({
							title: f,
