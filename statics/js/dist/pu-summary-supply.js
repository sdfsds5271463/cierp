...
function initEvent() {
	$("#refresh").on("click", function(a) {
		...
		a.preventDefault(), Business.verifyRight("PUREPORTPUR_PRINT") && window.print()
	}), $("#btn-export").click(function(a) {
		if (a.preventDefault(), Business.verifyRight("PUREPORTPUR_EXPORT")) {
			var b = {};
			for (var c in filterConditions) filterConditions[c] && (b[c] = filterConditions[c]);
			Business.getFile("../report/puDetail_supplyExporter?action=supplyExporter", b)
		}
		...
		if (a.preventDefault(), Business.verifyRight("PUREOORTDETAIL_QUERY")) {
			...
			parent.tab.addTabItem({
				...
				text: "採購明細表",
				url: "../report/puDetail_detail?action=detail&beginDate=" + filterConditions.beginDate + "&endDate=" + filterConditions.endDate + "&customerNo=" + b + "&goodsNo=" + c + "&storageNo=" + d
...
function reloadReport() {
	...
	window.location = "../report/puDetail_supply?action=supply" + a
}
