function init() {
	void 0 !== cRowId ? Public.ajaxPost("../basedata/contact/query?action=query", {
		...
		200 == a.status ? (rowData = a.data, initField(), initEvent(), initGrid(rowData.links)) : parent.$.dialog({
			title: "系統提示",
			content: "獲取客戶數據失敗，暫不能修改客戶，請稍候重試",
			icon: "alert.gif",
			max: !1,
			min: !1,
			cache: !1,
	...
	$_form.validator({
		...
		valid: function(a) {
			...
			Public.ajaxPost("../basedata/contact/checkName?action=checkName", {
				...
				-1 == a.status ? parent.$.dialog.confirm('客戶名稱 "' + b + '" 已經存在！是否繼續？', function() {
					postCustomerData()
...
function phoneCheck(a, b) {
	...
	return c.test(a) ? [!0, ""] : [!1, "請填寫正確的手機號碼"]
}
function telephoneCheck(a, b) {
	var c = /^([\d]+-){1,2}[\d]+$/;
	return c.test(a) ? [!0, ""] : [!1, "請填寫正確的座機號碼，格式為0754-1234567或086-0754-1234567。"]
}
