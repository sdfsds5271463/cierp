		...
		addEvent: function() {
			...
			if ($(".grid-wrap").on("click", ".ui-icon-pencil", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("TF_DELETE")) {
					var b = $(this).parent().data("id");
					$.dialog.confirm("您確定要刪除該調撥記錄嗎？", function() {
				...
				queryConditions.matchCon = "請輸入單據號或客戶名或備註" === a.$_matchCon.val() ? "" : a.$_matchCon.val(), queryConditions.beginDate = a.$_beginDate.val(), queryConditions.endDate = a.$_endDate.val(), queryConditions.outLocationId = -1, queryConditions.inLocationId = -1, THISPAGE.reloadData(queryConditions)
