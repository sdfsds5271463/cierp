define(["jquery", "print"], function(a, b, c) {
	...
	function e() {
		k("#refresh").on("click", function(a) {
			...
			a.preventDefault(), Business.verifyRight("PUREPORTPUR_PRINT") && k("div.ui-print").printTable()
		}), k("#btn-export").click(function(a) {
			a.preventDefault(), Business.verifyRight("PUREPORTPUR_EXPORT") && Business.getFile(n, m)
			...
			onCellSelect: function(a) {
				if (Business.verifyRight("PU_QUERY")) {
					...
					parent.tab.addTabItem({
						...
						text: "採購明細表",
						url: "../report/pu_detail_new?autoSearch=true&beginDate=" + m.beginDate + "&endDate=" + m.endDate + "&customerNo=" + c + "&goodsNo=" + d + "&storageNo=" + e
					})
				}
			},
	...
	var k = a("jquery"),
		...
		n = "../report/puDetail_supplyExporter?action=supplyExporter",
		o = "../report/puDetail_supply?action=supply";
	a("print");
	var p = Public.mod_PageConfig.init("puSummarySupplyNew");
	d(), e(), f();
	var q;
