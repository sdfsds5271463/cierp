define(["jquery", "print"], function(a) {
	function b() {
		...
		i("#filter-submit").on("click", function(b) {
			...
			b.preventDefault(), i("#filter-fromDate").val(j.beginDate), i("#filter-toDate").val(j.endDate), i("#filter-fromDeliveryDate").val(j.fromDeliveryDate), i("#filter-toDeliveryDate").val(j.toDeliveryDate), i("#filter-customer input").val(""), i("#filter-goods input").val(""), a.chkNot()
		})
	}
	function c() {
		i("#refresh").on("click", function(a) {
			...
			a.preventDefault(), Business.verifyRight("PURCHASEORDER_PRINT") && i("div.ui-print").printTable()
		}), i("#btn-export").click(function(a) {
			a.preventDefault(), Business.verifyRight("PURCHASEORDER_EXPORT") && Business.getFile("/report/purchaseOrder.do?action=detailExporter", j)
		})
	}
	function d() {
		...
		i("#grid").jqGrid({
			url: "../report/purchaseOrder?action=detail",
			postData: j,
			datatype: "json",
			autowidth: !0,
			height: a,
