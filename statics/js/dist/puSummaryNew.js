define(["jquery", "print"], function(a, b, c) {
	function d() {
		Business.filterSupplier(), Business.filterGoods(), Business.filterStorage(), Business.moreFilterEvent(), k("#conditions-trigger").trigger("click"), k("#filter-fromDate").val(m.beginDate || ""), k("#filter-toDate").val(m.endDate || ""), k("#filter-customer input").val(m.customerNo || ""), k("#filter-goods input").val(m.goodsNo || ""), k("#filter-storage input").val(m.storageNo || ""), m.beginDate && m.endDate && (k("#selected-period").text(m.beginDate + "至" + m.endDate), k("div.grid-subtitle").text("日期: " + m.beginDate + " 至 " + m.endDate)), k("#filter-fromDate, #filter-toDate").datepicker(), chkboxes = k("#chk-wrap").cssCheckbox(), k("#filter-submit").on("click", function(a) {
			a.preventDefault();
			...
			j(), k("#filter-menu").removeClass("ui-btn-menu-cur")
		}), k("#filter-reset").on("click", function(a) {
			a.preventDefault(), k("#filter-fromDate").val(m.beginDate), k("#filter-toDate").val(m.endDate), k("#filter-customer input").val(""), k("#filter-goods input").val(""), k("#filter-storage input").val(""), m.customerNo = "", m.goodsNo = "", m.storageNo = "", chkboxes.chkNot()
		})
	}
	function e() {
		k("#refresh").on("click", function(a) {
			...
			a.preventDefault(), Business.verifyRight("PUREPORTINV_PRINT") && k("div.ui-print").printTable()
		}), k("#btn-export").click(function(a) {
			a.preventDefault(), Business.verifyRight("PUREPORTINV_EXPORT") && Business.getFile(n, m)
			...
			onCellSelect: function(a) {
				if (Business.verifyRight("PU_QUERY")) {
					var b = k("#grid").getRowData(a),
						c = k("#supplierAuto").val(),
						d = b.invNo,
						e = b.locationNo;
					parent.tab.addTabItem({
						tabid: "report-puDetail",
						text: "採購明細表",
						url: "../report/pu_detail_new?autoSearch=true&beginDate=" + m.beginDate + "&endDate=" + m.endDate + "&customerNo=" + c + "&goodsNo=" + d + "&storageNo=" + e + "&showSku=" + m.showSku
					})
				}
			},
	...
	var k = a("jquery"),
		...
		n = "../report/puDetail_invExporter?action=invExporter",
		o = "../report/puDetail_inv?action=inv";
	a("print");
	var p = Public.mod_PageConfig.init("puSummaryNew");
	d(), e(), f();
	var q;
