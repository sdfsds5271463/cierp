...
function(a) {
	function b() {
		a("#assisting-list").on("mouseover", ">li", function() {
			...
			if (b.preventDefault(), Business.verifyRight("FZSX_DELETE")) {
				...
				a.dialog.confirm("刪除后不可恢復！您確定要刪除該輔助屬性嗎？", function() {
					e(c)
				})
			}
	...
	function c(b, c) {
		var e = "add" == b ? "新增分類" : "編輯分類",
			f = "add" == b ? ["儲存", "關閉"] : ["確定", "取消"],
			g = c ? c.id : void 0,
			i = "edit" == b ? c.name : "",
			j = ['<div class="manage-wrap assisting-manage" id="manage-wrap">', '<form action="#" id="manage-form">', '<ul class="mod-form-rows">', '<li class="row-item">', '<div class="label-wrap fl">', '<label for="assistingName">名稱：</label>', "</div>", '<div class="ctn-wrap fl">', '<input type="text" id="assistingName" name="assistingName" class="ui-input" value="' + i + '" />', "</div>", "</li>", "</ul>", "</form>", "<div>"].join("");
		h = a.dialog({
			title: e,
			width: 320,
			height: 100,
	...
	function g(b) {
		for (var c = '<li class="custom"><a href="#" data-id="#{id}" class="item"><i></i><strong>#{name}</strong></a><span class="operation"><a href="#" class="btn-edit ui-icon-edit" title="編輯">編輯</a><a href="#" class="btn-del ui-icon-del" title="刪除">刪除</a></span></li>', d = [], e = 0, f = b.length; f > e; e++) {
			var g = b[e];
