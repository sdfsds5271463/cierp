define(["jquery", "print"], function(a, b, c) {
	function d() {
		m.matchCon ? k("#matchCon").val(m.matchCon || "請輸入客戶、供應商或編號查詢") : (k("#matchCon").addClass("ui-input-ph"), k("#matchCon").placeholder()), m.customer && k("#customer").attr("checked", !0), m.supplier && k("#supplier").attr("checked", !0), k("#search").on("click", function(a) {
			a.preventDefault();
			var b = "請輸入客戶、供應商或編號查詢" === k("#matchCon").val() ? "" : k.trim(k("#matchCon").val());
			m = {
				matchCon: b,
