	...
	THISPAGE = {
		...
		initDom: function(a) {
			...
			var c = '<a id="savaAndAdd" class="ui-btn ui-btn-sp">儲存並新增</a><a id="save" class="ui-btn">儲存</a>',
				d = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invPu/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">列印</a><a id="edit" class="ui-btn">儲存</a>',
				e = '<a id="add" class="ui-btn ui-btn-sp">新增</a><a href="../scm/invPu/toPdf?action=toPdf&id=' + a.id + '" target="_blank" id="print" class="ui-btn">列印</a>',
				f = "",
				g = "";
			billRequiredCheck ? (f = '<a class="ui-btn" id="audit">審覈</a>', g = '<a class="ui-btn" id="reAudit">反審覈</a>') : this.$_checkName.parent().hide();
		...
		addEvent: function() {
			...
			this.$_date.bind("keydown", function(a) {
				...
				if (a.preventDefault(), Business.verifyRight("PU_ADD")) {
					var b = "購貨單",
						c = "purchase-purchase";
					if ("150502" == originalData.transType) var b = "購貨退貨單",
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-prev-dis") ? (parent.Public.tips({
					type: 2,
					content: "已經沒有上一張了！"
				}), !1) : (a.idPostion = a.idPostion - 1, 0 === a.idPostion && $(this).addClass("ui-btn-prev-dis"), loading = $.dialog.tips("數據載入中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invPu/update?action=update", {
				...
				return b.preventDefault(), $(this).hasClass("ui-btn-next-dis") ? (parent.Public.tips({
					type: 2,
					content: "已經沒有下一張了！"
				}), !1) : (a.idPostion = a.idPostion + 1, a.idLength === a.idPostion + 1 && $(this).addClass("ui-btn-next-dis"), loading = $.dialog.tips("數據載入中...", 1e3, "loading.gif", !0), Public.ajaxGet("../scm/invPu/update?action=update", {
					id: a.idList[a.idPostion]
				}, function(b) {
		...
		getPostData: function(a) {
			...
			if (f.length > 0) {
				...
				if (taxRequiredCheck && (h.totalTax = $("#grid").jqGrid("footerData", "get").tax.replace(/,/g, ""), h.totalTaxAmount = $("#grid").jqGrid("footerData", "get").taxAmount.replace(/,/g, "")), requiredMoney) {
					...
					var i = "150501" == h.transType ? "付款額" : "退款額";
