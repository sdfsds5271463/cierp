define(["jquery", "print"], function(a, b, c) {
	function d() {
		q.cssCheckbox(), "true" == m.showDetail && (q.find("label").addClass("checked"), r[0].checked = !0), m.beginDate && m.endDate && k("div.grid-subtitle").text("日期: " + m.beginDate + "至" + m.endDate), k("#filter-fromDate").val(m.beginDate), k("#filter-toDate").val(m.endDate), k("#customer input").val(m.customerName), Business.supplierCombo(k("#customer"), {
			defaultSelected: 0,
			addOptions: {
				text: t,
				value: 0
		...
		k("#filter-submit").on("click", function(c) {
			...
			return g.getTime() > h.getTime() ? void parent.Public.tips({
				...
				supplierName: l,
				showDetail: n
			}, k("div.grid-subtitle").html("<p>供應商：" + l + "</p><p>日期: " + e + " 至 " + f + "</p>"), void j(o))
		}), k("#filter-reset").on("click", function(a) {
			a.preventDefault(), k("#filter-fromDate").val(""), k("#filter-toDate").val(""), k("#customer input").val("")
		})
	}
	function e() {
		k("#btn-print").click(function(a) {
			...
			if (k(this).data("hasInstance")) this.customerDialog.show().zindex();
				...
				var b = k("#customer").prev().text().slice(0, -1),
					c = "選擇" + b;
				if ("供應商" === b || "購貨單位" === b) var d = "url:../settings/select_customer?type=10&multiselect=false";
	...
	var k = a("jquery"),
		...
		n = "../report/supplierBalance_exporter?action=exporter",
		o = "../report/supplierBalance_detail?action=detail",
		p = k("#customer"),
		q = k("#match"),
