function initDom() {
	function a(a, b, c) {
		conditions.typeNumber = a, conditions.name = b, c || $("#grid").setGridParam({
			postData: conditions
		}).trigger("reloadGrid"), parent.$("li.l-selected a").eq(0).text(conditions.name + "類別")
	}
	...
	1 == g.length ? (g.addClass("cur"), b = 0) : (b = ["number", typeNumber], $("#custom-assisting").parent().addClass("cur")), a(typeNumber, e[typeNumber], !0), $("#custom-assisting").combo({
		...
		ajaxOptions: {
			formatData: function(a) {
				...
				for (var b = 0, c = a.length; c > b; b++) a[b].name = a[b].name.replace("類別", ""), e[a[b].number] && (a.splice(b, 1), b--, c--);
...
function initEvent() {
	$("#btn-add").click(function(a) {
		...
		conditions.skey = "輸入類別名稱查詢" == b ? "" : b, $("#grid").setGridParam({
			postData: conditions
		}).trigger("reloadGrid")
	...
	$("#grid").jqGrid({
		...
		loadComplete: function(a) {
			if (a && 200 == a.status) {
				...
				var f = 250 == a.status ? "沒有" + conditions.name + "類別數據！" : "獲取" + conditions.name + "類別數據失敗！" + a.msg;
				parent.Public.tips({
					type: 2,
					content: f
				})
...
function postData(a) {
	...
	var f = {
		...
		g = "add" == d ? "新增" + conditions.name + "類別" : "修改" + conditions.name + "類別";
...
var conditions = {
	...
	handle = {
		operate: function(a, b) {
			if ("add" == a) {
				var c = "新增" + conditions.name + "類別";
				({
					oper: a,
					callback: this.callback
				})
			} else {
				var c = "修改" + conditions.name + "類別";
				({
					oper: a,
			...
			var d = ['<form id="manage-form" action="">', '<ul class="mod-form-rows manage-wrap" id="manager">', '<li class="row-item" style="position:relative; display:none;">', '<div class="label-wrap"><label for="ParentCategory">上級分類:</label></div>', '<div class="ctn-wrap" style="position:relative;"><input type="text" value="" class="ui-input" name="ParentCategory" id="ParentCategory" readonly></div>', '<div class="dn hideFeild"></div>', "</li>", '<li class="row-item">', '<div class="label-wrap"><label for="category">類別:</label></div>', '<div class="ctn-wrap"><input type="text" value="" class="ui-input" name="category" id="category"></div>', "</li>", "</ul>", "</form>"],
				e = 90;
		...
		del: function(a) {
			$.dialog.confirm("刪除的" + conditions.name + "類別將不能恢復，請確認是否刪除？", function() {
				Public.ajaxPost("../basedata/assist/delete?action=delete", {
					...
					if (b && 200 == b.status) {
						parent.Public.tips({
							content: "刪除" + conditions.name + "類別成功！"
						...
						type: 1,
						content: "刪除" + conditions.name + "類別失敗！" + b.msg
					})
				})
			})
		},
