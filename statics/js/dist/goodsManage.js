...
function postCustomerData() {
	if ("add" == oper) {
		...
		Public.ajaxPost("../basedata/inventory/checkName?action=checkName", {
			...
			-1 == b.status ? $.dialog.confirm('商品名稱 "' + a + '" 已經存在！是否繼續？', function() {
				postData()
			}) : postData()
		})
	} else postData()
...
function initCombo() {
	storageCombo = $("#storage").combo({
		...
		extraListHtml: '<a href="javascript:void(0);" id="quickAddSkuProp" class="quick-add-link"><i class="ui-icon-add"></i>新增屬性</a>',
		text: "name",
		value: "id",
		defaultSelected: -1,
		editable: !0,
...
function initEvent() {
	...
	categoryTree = Public.categoryTree($category, b), $("#specs").blur(function() {
		...
		if (Business.verifyRight("FZSX_ADD")) {
			var b = function() {
				...
				c = ['<div class="manage-wrap assisting-manage" id="manage-wrap">', '<form action="#" id="manage-form">', '<ul class="mod-form-rows">', '<li class="row-item">', '<div class="label-wrap fl">', '<label for="assistingName">名稱：</label>', "</div>", '<div class="ctn-wrap fl">', '<input type="text" id="assistingName" name="assistingName" class="ui-input" value="" />', "</div>", "</li>", "</ul>", "</form>", "<div>"].join("");
			manageDialog = $.dialog({
				title: "新增分類",
				width: 320,
				height: 100,
	...
	$("#grid").on("click", ".ui-icon-plus", function(a) {
		...
		if (c && c.skuId && "edit" == oper) $.dialog.confirm("刪除的分錄將不能恢復，請確認是否刪除？", function() {
		...
		})) : (a.preventDefault(), $.dialog.confirm("取消後會清空預警表的屬性列，是否繼續？", function() {
								...
								callback: function(b, c) {
									...
									if ($("#" + a).siblings(".jqgrow").each(function(a) {
										...
										if (c) for (var a = c.length - 1; a >= 0; a--) for (var e = b.length - 1; e >= 0; e--) b[e].serNum === c[a].serNum && (defaultPage.Public.tips({
											type: 2,
											content: "期初分錄中已存在序列號：[" + b[e].serNum + "]"
										}), d = !1)
									}), d) {
										if (b.length) {
											$("#" + a).data("serNumInfo", b);
		...
		THISPAGE.warningGrid = $gridWarning.jqGrid({
			...
			gridComplete: function(b) {
				...
				$("#batchHighQty,#batchLowQty").powerFloat({
					...
					reverseSharp: !0,
					target: function() {
						return cancleGridEdit(), _self = this, $t = $('<label><input class="ui-input mrb" type="text"/><span class="ui-btn ui-btn-sp" data-tid="' + this[0].id + '">確定</span></label>'), $t.appendTo($("#patchInputBox").html("")), $t.find(".ui-input").keypress(function(a) {
							Public.numerical(a)
...
function floatCheck(a, b) {
	var c = /^[0-9\.]+$/,
		a = $.trim(a);
	return "quantity" == b ? b = "期初數量" : "unitCost" == b && (b = "單位成本"), c.test(a) ? [!0, ""] : "" == a ? [!1, b + "不能為空！（如果不需要該行數據，可以刪除行）"] : [!1, "請填寫正確的" + b]
}
...
function initValidator() {
	...
	$form.validator({
		rules: {
			code: [a, "商品編號只能包含<,>,&,,'字元組成"],
			number: function(a, b) {
				var c = $(a).val();
				try {
					return c = Number(c), c >= 0 ? ($(a).val(c), !0) : "欄位不合法！請輸入數值"
				} catch (d) {
					return "欄位不合法！請輸入數值"
				}
			},
			checkCode: function(a, b) {
				...
				return $.ajax({
					...
					success: function(a, b) {
						return a ? void(c = -1 == a.status ? rowData && rowData.barCode === c ? !0 : "商品條碼已經存在！" : !0) : !1
					},
					error: function(a) {
						c = "遠端數據校驗失敗！"
					}
