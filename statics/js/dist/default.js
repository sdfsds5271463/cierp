...
function BBSPop() {
	var a = $("#yswb-tab"),
		b = ['<ul id="yswbPop">', '<li class="yswbPop-title">請選擇您要進入的論壇</li>', '<li><strong>產品服務論壇</strong><p>線上會計，線上進銷存操作問題諮詢</p><a href="http://wpa.qq.com/msgrd?V=3&uin=209887082&Site=青城&Menu=yes" target="_blank">進入論壇>></a></li>', '<li><strong>會計交流論壇</strong><p>會計實操，會計學習，會計資訊</p><a href="http://wpa.qq.com/msgrd?V=3&uin=209887082&Site=青城&Menu=yes" target="_blank">進入論壇>></a></li>', "</ul>"].join("");
...
$("#nav").on("click", "[rel=pageTab]", function(a) {
	...
	if (2 === SYSTEM.siVersion && SYSTEM.isOpen) {
		var b, c = location.protocol + "//" + location.host + "/update_info.jsp",
			d = '您的單據分錄已經錄入達到300條，繼續使用選擇<a href="http://wpa.qq.com/msgrd?V=3&uin=209887082&Site=青城&Menu=yes" target="_blank">購買產品</a>或者完善個人資訊贈送1000條免費容量。';
	...
	if ($("#scollUp").click(function() {
		...
		a.preventDefault(), $.dialog({
			...
			height: 180,
			title: "系統提示",
			content: '<div class="re-initialize"><h3>重新初始化系統將會清空你錄入的所有數據，請慎重！</h3><ul><li>系統將刪除您新增的所有商品、供應商、客戶</li><li>系統將刪除您錄入的所有單據</li><li>系統將刪除您錄入的所有初始化數據</li></ul><p><input type="checkbox" id="understand" /><label for="understand">我已清楚瞭解將產生的後果</label></p><p class="check-confirm">（請先確認並勾選「我已清楚瞭解將產生的後果」）</p></div>',
			icon: "alert.gif",
			okVal: "重新初始化",
			ok: function() {
				if ($("#understand").is(":checked")) {
					this.close();
					var a = $.dialog.tips("正在重新初始化，請稍候...", 1e3, "loading.gif", !0).show();
					$.ajax({
						type: "GET",
		...
		var c = [{
			...
			d = ['<div class="ui-dialog-tips">', "<p>謝謝您使用本產品，您的當前服務已經到期，到期3個月后數據將被自動清除，如需繼續使用請購買/續費！</p>", '<p style="color:#AAA; font-size:12px;">(續費後請重新整理頁面或重新登錄。)</p>', "</div>"].join("");
		$.dialog({
			width: 400,
			min: !1,
			max: !1,
