...
$.fn.extend({
	...
	ajaxPost:function(url, params, callback, errCallback){
		...
		$.extend(true, ajaxOpts, {
			...
			error: function(err){  
				parent.Public.tips({type: 2, content : '操作無法成功，請稍後重試！'});
				errCallback && errCallback(err);
		   	}
		});
...
Business.linkCombo = function(page, opts){
	var opts = $.extend(true, {
		data: function(){
			if(defaultPage.SYSTEM.linkinfo) {
				return defaultPage.SYSTEM.linkinfo;
			} else {
				return '../basedata/contact/getLinkMen?buId='+defaultPage.SYSTEM.mbuId;
			}
		},
		width: 120,
		height: 300,
		...
		callback: {
			onChange: function(data){
				...
				if(data){
					var linkMan = data;
					page.$linkPhone.find("input").val(linkMan.linkMobile?linkMan.linkMobile:linkMan.linkPhone);
					page.$linkAddress.find("input").val(linkMan.province+linkMan.city+linkMan.county+linkMan.address);
				}else{
					page.$linkPhone.find("input").val('');
					page.$linkAddress.find("input").val('');
				}
			}
		}
	}, opts);
...
Business.serNumManage = function(opts){
	...
	var params = {
		width: 650,
		height: 400,
		title: (serNumUsedList ? '修改' : '錄入') + '【'+data.name+'】的序列號',
		...
		esc:false,//外掛的BUG，esc不觸發cancel事件
		onClose:function(){
			//console.log(111)
		}
	};
	if(enableStorage){
		//如果是銷售單等使用序列號新增商品時， 或者改變倉庫時
		params.title = '選擇【'+data.name+'】的序列號',
	...
	$('.grid-wrap').on('click', '.goods .ui-icon-ellipsis', function(e){
		...
		$.dialog({
				...
				data: {
					...
					callback: function(newId, curID, curRow){
						if(curID === '') {
							$("#gridCombination").jqGrid('addRowData', newId, {}, 'last');
							_self.newSonId = newId + 1;
						};
						setTimeout( function() { $("#gridCombination").jqGrid("editCell", curRow, 2, true) }, 10);
					}
				},
				lock: true,
				button:[{name: '選中',defClass:'ui_state_highlight fl', callback: function () {
					this.content.callgoodback();
			        return false;
				}},
				{name: '選中並關閉',defClass:'ui_state_highlight', callback: function () {
					this.content.callgoodback();
